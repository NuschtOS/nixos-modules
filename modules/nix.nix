    ...
    system = {
      activationScripts = {
        deleteChannels = lib.mkIf cfg.deleteChannels /* bash */ ''
          ...
          rm -rfv /root/{.nix-channels,.nix-defexpr} /home/*/.local/state/nix/defexpr /home/*/{.nix-channels,.nix-defexpr} /nix/var/nix/profiles/per-user/*/channels* || true
        '';

        deleteUserProfiles = lib.mkIf cfg.deleteUserProfiles /* bash */ ''
          echo "Deleting all user profiles..."
          rm -rfv /root/.nix-profile /home/*/.local/state/nix/profile /home/*/.nix-profile /nix/var/nix/profiles/per-user/*/profile* || true
        '';
